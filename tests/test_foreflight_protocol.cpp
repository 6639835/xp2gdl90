...
TEST_CASE(
  ...
  ASSERT_TRUE(
      !xp2gdl90::foreflight::ParseDiscoveryBroadcast(empty_packet, &port));
  ASSERT_TRUE(!xp2gdl90::foreflight::ParseDiscoveryBroadcast(
      std::vector<uint8_t>{'{', '}'}, nullptr));

  const std::vector<uint8_t> array_root{'[', '1', ']'};
  ASSERT_TRUE(
      !xp2gdl90::foreflight::ParseDiscoveryBroadcast(array_root, &port));

  const std::vector<uint8_t> missing_gdl90{'{', '"', 'A', 'p', 'p', '"', ':',
                                           '"', 'F', 'o', 'r', 'e', 'F', 'l',
                                           'i', 'g', 'h', 't', '"', '}'};
  ASSERT_TRUE(
      !xp2gdl90::foreflight::ParseDiscoveryBroadcast(missing_gdl90, &port));
