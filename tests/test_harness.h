...
#define TEST_CASE_IMPL(name, id)                                               \
  ...
  static test_harness::Registrar TEST_CONCAT(test_reg_, id)(                   \
      name, TEST_CONCAT(test_fn_, id));                                        \
  static void TEST_CONCAT(test_fn_, id)()
...
#define ASSERT_TRUE(condition)                                                 \
  do {                                                                         \
    if (!(condition)) {                                                        \
      throw test_harness::AssertionFailure(test_harness::FormatMessage(        \
          __FILE__, __LINE__, "ASSERT_TRUE failed: " #condition));             \
    }                                                                          \
  } while (0)
  ...
  do {                                                                         \
    if (!((expected) == (actual))) {                                           \
      ...
      _assert_eq_oss << "ASSERT_EQ failed: expected '" << (expected)           \
                     << "' got '" << (actual) << "'";                          \
      throw test_harness::AssertionFailure(test_harness::FormatMessage(        \
          __FILE__, __LINE__, _assert_eq_oss.str()));                          \
    }                                                                          \
  } while (0)
  ...
  do {                                                                         \
    if ((expected) == (actual)) {                                              \
      ...
      _assert_ne_oss << "ASSERT_NE failed: did not expect '" << (expected)     \
                     << "'";                                                   \
      throw test_harness::AssertionFailure(test_harness::FormatMessage(        \
          __FILE__, __LINE__, _assert_ne_oss.str()));                          \
    }                                                                          \
  } while (0)
